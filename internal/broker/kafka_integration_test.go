...
func TestHandlerError_NoCommit(t *testing.T) {
	b := testutil.SetupKafka(t)
	topic := "test-error-" + ulid.Make().String()

	// Channel to track handler calls
	handlerCalls := make(chan int, 10)
	callCount := 0

	ctx, cancel := context.WithTimeout(context.Background(), 30*time.Second)
	defer cancel()

	// Start consumer that fails first time
	go func() {
		err := b.Subscribe(ctx, topic, "test-group-"+ulid.Make().String(), func(ctx context.Context, msg *broker.Message) error {
			callCount++
			handlerCalls <- callCount
	...
	// Wait for handler calls
	// Should be called at least twice (first failure, then retry)
	...
	for len(calls) < 2 {
		...
		case <-timeout:
			t.Fatalf("expected at least 2 handler calls, got %d", len(calls))
		}
	}

	assert.GreaterOrEqual(t, len(calls), 2, "handler should be called at least twice due to error")
}
